        ...
        <div className="page-wrap">
          ...
          {activeSection === 'My history' && (
            <section className="history-view">
              ...
              {healthStories.length === 0 && documents.length === 0 ? <div className="empty-doc panel"><History size={26} /><strong>{t('history.empty')}</strong><button className="text-button" onClick={openIntake}>{t('history.start')} <ArrowRight size={14} /></button></div> : <div className="history-list">{healthStories.map((s) => <div className="record-item history-item" key={s.id} onClick={() => openReport(s.id)}><div className="record-icon green"><FileSignature size={18} /></div><div><strong>{t('history.storyTitle')} · {s.status === 'flagged' ? t('summary.priorityAlert') : t('activity.available')}</strong><span>{t('history.completedOn')} {new Date(s.created_at).toLocaleDateString()}</span></div><span className={`status-pill ${s.status === 'complete' ? 'complete' : ''}`}>{s.status === 'flagged' ? t('activity.inProgress') : t('activity.available')}</span><button className="text-button" onClick={(e) => { e.stopPropagation(); openReport(s.id); }}>{t('report.viewReport')} <ArrowRight size={14} /></button></div>)}{documents.map((document) => <div className="record-item history-item" key={`document-${document.id}`} onClick={() => openDocumentViewer(document)}><div className="record-icon green"><FileCheck2 size={18} /></div><div><strong>Document uploaded · {document.filename}</strong><span>Uploaded on {new Date(document.created_at).toLocaleDateString()} · {document.ocr_status === 'processed' ? 'Digitised' : document.ocr_status === 'failed' ? 'Digitisation needs attention' : 'Processing'}</span></div><span className={`status-pill ${document.ocr_status === 'processed' ? 'complete' : ''}`}>{document.ocr_status === 'processed' ? 'Available' : 'Uploaded'}</span><button className="text-button" onClick={(event) => { event.stopPropagation(); openDocumentViewer(document); }}>View <ArrowRight size={14} /></button></div>)}</div>}
            </section>
          )}
      ...
      {modal === 'upload' && (
        <div className="modal-backdrop" onClick={() => setModal(null)}>
          <div className="upload-modal" onClick={(e) => e.stopPropagation()} role="dialog" aria-modal="true">
            ...
            <label className="modal-consent"><input type="checkbox" checked={uploadConsent} onChange={(event) => setUploadConsent(event.target.checked)} /><span className={uploadConsent ? 'check-box checked' : 'check-box'}>{uploadConsent && <Check size={13} />}</span><span>{t('upload.consent')}</span></label>
