...
import { supabase, type Patient, type MedicalDocument, type ActivityLog, type ConsentRecord, type HealthStory, type HealthReport } from '@/lib/supabase';
...
export default function App() {
  ...
  const loadReports = useCallback(async () => {
    if (!patientId) return;
    const { data } = await supabase.from('health_reports').select('*').eq('patient_id', patientId).order('created_at', { ascending: false });
    if (data) setHealthReports(data as HealthReport[]);
  }, [patientId]);
